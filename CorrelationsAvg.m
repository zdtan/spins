...
% save in 'folder'
% average over starting point in chain for 'api-col'
...
switch CODE
    ...
    case 'api-col'
        ...
        % average over choice of starting spin too (using circshift)
        sites = [1:2*M];
        samples = [1:K*L*length(sites)];
        dotp = zeros(length(sites),length(samples));
        % settle sample by sample
        for i = [1:K]
            for j = [1:L]
                ...
                for startpt = [1:length(sites)] %error!: startpt = [1:length(sites)-1]
                    % circshift tempspins to start with different spin each time
                    tempspins = circshift(tempspins,1); %circshift(tempspins,1,1);
                    dotp(:,(startpt-1)*K*L+(i+(j-1)*K)) = dot(tempspins,repmat(tempspins(1,:),length(sites),1),2);
                end
            end
        end
    case 'kag-col'
...
ylabel('\langle S_0 \cdot S_i \rangle','FontSize',30)
grid on
print(h,'-dpng',[folder,'/LOG-correlations-avg_',CODE,'.png']);
close
...
xlabel('i','FontSize',30)
ylabel('S_0 \cdot S_i','FontSize',30)
print(h,'-dpng',[folder,'/LOG-correlations-spread_',CODE,'.png']);
close
...
if length(sites) > 1
    if length(sites) == 2
        ...
        % this does NOT average over starting spin for 'api-col'
        dotpnn = horzcat(dotp(2,:),dotp(length(sites),:));
    end
    ...
    % log scale also
    h = figure;
    semilogy(sort(dotpnn));
    set(gca,'FontSize',20);
    xlabel('nearest neighbours','FontSize',30)
    ylabel('S_0 \cdot S_1','FontSize',30)
    print(h,'-dpng',[folder,'/LOG-correlations-nn_',CODE,'.png']);
    close
else

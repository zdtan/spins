%% based on GenerateTS and does MC directly at target temperature
% takes initial spin configuration as input

function [ spinMat, energy, acc ] = GenerateTS2( spinMat, ID, ...
...
%% Monte Carlo at target temperature
...
save([spinfile,'.mat'],'spinMat','acc','sinit',...
	'alpha','beta','sigma','K','L','M','constrainM','-mat');
...
energyfile = [ID,'/energy','-a',num2str(alpha),'-b',num2str(beta), ...
    		'-d',num2str(delta),'-s',num2str(sigma), ...
    		'-',num2str(K),'x',num2str(L),'x',num2str(M), ...
    		'_',num2str(numSweep),'sweeps_',num2str(acc),'acc'];
if ~AFM
	energyfile = [energyfile,'_FM'];
end
save([energyfile,'.mat'],'energy','-mat');
